...
Throughout the past 5 seasons for three-point shooting has remained
relatively the same, but we can see some changes in the graph. While the
2020-2021 season has the highest Q3 out of all the seasons, we can see
that surprisingly the 2023-2024 season has the highest average
three-point percentage. Another trend is while the means have stayed
mostly the same, the Q1 and Q3 for this most recent season has tightened
up meaning more players have a three-point percentage closer to the
average.

``` r
master %>%
  filter(FG3_PCT > 0) %>%
  ggplot(aes(x = SEASON, y = FG3_PCT, fill = SEASON)) +
  geom_boxplot(outlier.alpha = 0.2) + 
  coord_cartesian(ylim = c(0.2, 0.45)) + 
  labs(title = "Three-Point Percentage by Season",
       x = "Season", y = "Three-Point Percentage", fill = "Season") +
...
![](README_files/figure-gfm/unnamed-chunk-25-1.png)<!-- --> Luka takes a
lot of very difficult 3 point shots by himself to try and save the
offense, this makes his FG_PCT appear weaker than it is. Otherwise, he
is consistently playing at an MVP level and getting ROBBED

## Conclusion

In conclusion, our analysis of the NBA as a whole and individuals has
shown that just giving an eye test won’t answer all questions. Looking
into the data had helped us all look at the NBA a bit different with
more insight on trends. The reality of the NBA is that when comparing
players, there really aren’t two of the same player, especially the ones
near stardom. Comparing average players is a completely different game
then comparing the best of the best.

We found it super interesting that for the plus/minus comparisons, that
nearly the entire starting roster for the Thunder was present. With Shai
top right, and his teammates top left. Meaning that he did the heavy
lifting in scoring and his team really didn’t get all that many chances
around him.

We also think it was a great idea to have a specific player be a
baseline by himself, Luka Doncic. The stats tell a pretty good story,
that mvp’s aren’t only stat reliant, players like himself honestly can’t
really be compared to many others if any, and that he is maybe the most
robbed player of all time.

If we were to do any future research on the NBA, team statistics would
be the way to go. We did a major focus on individual performances and
neglected team activity. Which we don’t think is necessarily a good or
bad decision, but was at least a good focus for the project.
